...
export const ENGINE_VERSION = "3.0-public-evidence";
...
async function browserProbe(context, normalized, options) {
...
async function checkOne(context, rawLink, options) {
  ...
  const [shortProbe, directProbe, mobileProbe, browser] = await Promise.all([
    ...
    browserProbe(context, normalized, options)
  ]);

  const evidence = buildEvidence([shortProbe, directProbe, mobileProbe], browser);
  ...
  return makeResult(rawLink, normalized, classification, {
    ...
    httpStatus: browser.navigationStatus ?? evidence.statuses.find(Number.isFinite) ?? null,
    durationMs: Date.now() - started,
...
export async function checkRedeemLinks(rawLinks) {
  ...
  async function worker() {
    while (true) {
      ...
      try {
        results[index] = await checkOne(runtime.context, rawLinks[index], options);
      } catch (error) {
...
export async function testPublicEngine() {
  ...
  try {
    ...
    let status = null;
    try {
      const response = await page.goto("https://one.google.com/", { waitUntil: "domcontentloaded", timeout: 15000 });
