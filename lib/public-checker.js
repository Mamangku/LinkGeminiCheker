...
export const ENGINE_VERSION = "3.2-serviceactivation";
...
/*
 * Google Payments Reseller Subscription / Managed Signup
 * https://serviceactivation.google.com/subscription/new/<user_session_token>
 *
 * Status di bawah hanya dipakai bila marker benar-benar muncul pada response
 * publik Google. Bot tidak menganggap redirect/login sebagai Used atau Valid.
 */
const SERVICE_USED_PATTERNS = [
  "this activation link has already been used",
  "activation link has already been used",
  "this subscription has already been activated",
  ...
  "end user already entitled",
  "session has already been used",
  "user session has already been used",
...
const SERVICE_EXPIRED_PATTERNS = [
...
const SERVICE_INVALID_PATTERNS = [
  ...
  "cannot process the request because it is malformed",
...
function extractServiceActivation(url) {
  const host = url.hostname.toLowerCase();
  if (host !== "serviceactivation.google.com") return null;

  const match = url.pathname.match(
    /^\/subscription\/(new|entitle)\/([A-Za-z0-9_-]+={0,2})\/?$/
  );
  ...
  // Token ini opaque. Jangan menebak isi/expiry dari token.
  // Validasi hanya bentuk dasar agar typo jelas dapat ditolak.
...
export function normalizeRedeemLink(rawLink) {
  ...
  if (!isAllowedHost(url.hostname)) {
    return { ok: false, status: STATUS.INVALID, reason: "Bukan domain Google redeem yang didukung" };
  }

  const service = extractServiceActivation(url);
  if (service) {
  ...
  if (url.hostname.toLowerCase() === "serviceactivation.google.com") {
    return {
      ok: false,
      status: STATUS.INVALID,
      reason: "Format link serviceactivation Google tidak sesuai"
    };
  }
  ...
  return {
    ok: false,
    status: STATUS.INVALID,
    reason: "Format link referral Google tidak dikenali"
  };
}
...
async function browserProbe(browser, normalized, options) {
  ...
  const url = normalized.kind === "serviceactivation"
    ? normalized.canonicalUrl
    : `${normalized.canonicalUrl}?g1_landing_page=5&hl=en&otzr=1`;
  try {
...
export function classifyEvidence(evidence, options = {}) {
  ...
  const serviceActivation = options.linkKind === "serviceactivation";
  const serviceUsed = serviceActivation && containsAny(text, SERVICE_USED_PATTERNS);
  const serviceExpired = serviceActivation && containsAny(text, SERVICE_EXPIRED_PATTERNS);
  const serviceInvalid = serviceActivation && containsAny(text, SERVICE_INVALID_PATTERNS);
  const serviceValid = serviceActivation && containsAny(text, SERVICE_VALID_PATTERNS);

  if (serviceUsed) signals.push("serviceactivation_used");
  if (serviceExpired) signals.push("serviceactivation_expired");
  if (serviceInvalid) signals.push("serviceactivation_invalid");
  if (serviceValid) signals.push("serviceactivation_activation_ui");
  ...
  // serviceactivation.google.com memakai opaque user-session token.
  // Hanya marker Google yang eksplisit yang boleh membedakan Used/Expired/Invalid.
  if (serviceUsed) {
    return {
      status: STATUS.USED,
      confidence: "high",
      reason: "Google menandai subscription/session ini sudah diaktivasi atau sudah digunakan",
      signals
    };
  }
  ...
  if (serviceExpired) {
    return {
      ...
      confidence: "high",
      reason: "Google menandai user-session/activation link sudah kedaluwarsa",
      signals
    };
  }
  ...
  if (serviceInvalid) {
    return {
      status: STATUS.INVALID,
      confidence: "high",
      reason: "Google menandai user-session token tidak valid atau malformed",
      signals
    };
  }
  ...
  // Untuk Managed Signup, activation UI yang benar-benar termuat adalah
  // bukti token diterima Google. Redirect login saja tidak cukup.
  if (serviceActivation && serviceValid && !evidence.loginGate) {
    return {
      status: STATUS.VALID,
      confidence: "medium",
      reason: "Google menerima user-session token dan menampilkan alur aktivasi subscription",
      signals
    };
  }
  ...
  if (evidence.loginGate) {
    return {
      status: STATUS.ERROR,
      confidence: "high",
      reason: serviceActivation
        ? "Google menerima navigasi tetapi meminta login sebelum status subscription dapat dibedakan; tidak aman menebak Valid/Used"
        : "Google meminta login sebelum membuka status offer; tidak ada bukti publik yang cukup untuk menebak",
      signals
    };
  }
...
async function checkOne(browser, rawLink, options) {
  ...
  if (normalized.kind === "serviceactivation") {
    // Jangan mengubah opaque user-session token dan jangan menambahkan query
    // yang tidak diperlukan. Dua probe hanya berbeda User-Agent.
    [probes, browserResult] = await Promise.all([
      Promise.all([
        fetchChain(normalized.canonicalUrl, options.timeoutMs, desktopUA),
        fetchChain(normalized.canonicalUrl, options.timeoutMs, mobileUA)
      ]),
      browserProbe(browser, normalized, options)
    ]);
  } else {
    const publicUrl = `${normalized.canonicalUrl}?g1_landing_page=5&hl=en&otzr=1`;
    let shortProbe, directProbe, mobileProbe;
    [shortProbe, directProbe, mobileProbe, browserResult] = await Promise.all([
    ...
    probes = [shortProbe, directProbe, mobileProbe];
  }

  const evidence = buildEvidence(probes, browserResult);
  const classification = classifyEvidence(evidence, {
    ...options,
    linkKind: normalized.kind
  });

  const allErrors = probes.map(probe => probe.error).filter(Boolean);
  if (allErrors.length === probes.length && classification.status === STATUS.ERROR && !evidence.corpus) {
